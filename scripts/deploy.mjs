...
const checkAwsAuth = () => {
  try {
    const out = runCapture('aws sts get-caller-identity');
    const id = JSON.parse(out);
    console.log(`✓ AWS identity: ${id.Arn}`);
  } catch {
    console.error('✗ AWS CLI not authenticated. Run `aws configure` or export credentials, then retry.');
    process.exit(1);
  }
};
...
run(
  `aws s3 sync ${DIST_DIR} s3://${BUCKET} --region ${REGION} --delete ` +
...
run(
  `aws s3 sync ${DIST_DIR} s3://${BUCKET} --region ${REGION} ` +
